...
SX126x::SX126x(int spiSelect, int reset, int busy, int interrupt) {
    ...
    gpio_init(SX126x_SPI_SELECT);
    gpio_set_dir(SX126x_SPI_SELECT, GPIO_OUT);

    gpio_init(SX126x_RESET);
    gpio_set_dir(SX126x_RESET, GPIO_OUT);

    gpio_init(SX126x_BUSY);
    gpio_set_dir(SX126x_BUSY, GPIO_IN);

    gpio_init(SX126x_INT0);
    gpio_set_dir(SX126x_INT0, GPIO_IN);

    gpio_init(9);  // SW pin, drive high always it seems
    gpio_set_dir(9, GPIO_OUT);
    gpio_put(9, 1);

    gpio_init(1);  // TX EN pin, not sure if it needs to be driven, active low
    ...
    spi_init(SPI_Peripheral, 2000000);

    spi_set_format(SPI_Peripheral,  // SPI instance
...
int16_t SX126x::begin(uint8_t packetType, uint32_t frequencyInHz,
    ...
    SetDio3AsTcxoCtrl(
        SX126X_DIO3_OUTPUT_3_3,
        RADIO_TCXO_SETUP_TIME << 6);  // convert from ms to SX126x time base
    ...
    SetDioIrqParams(SX126X_IRQ_ALL,  // all interrupts enabled
                    (SX126X_IRQ_RX_DONE | SX126X_IRQ_TX_DONE |
                     SX126X_IRQ_TIMEOUT),  // interrupts on DIO1
...
int16_t SX126x::LoRaConfig(uint8_t spreadingFactor, uint8_t bandwidth,
...
void SX126x::ReceiveStatus(uint8_t *rssiPacket, uint8_t *snrPacket) {
    ...
    (buf[1] < 128) ? (*snrPacket = buf[1] >> 2)
                   : (*snrPacket = ((buf[1] - 256) >> 2));
    *rssiPacket = -buf[0] >> 1;
}
...
void SX126x::Reset(void) {
    sleep_ms(10);
    gpio_put(SX126x_RESET, false);
    sleep_ms(20);
    gpio_put(SX126x_RESET, true);
    sleep_ms(10);
    while (gpio_get(SX126x_BUSY))
        ;
}

void SX126x::Wakeup(void) { GetStatus(); }

//----------------------------------------------------------------------------------------------------------------------------
//  The command SetStandby(...) is used to set the device in a configuration
//  mode which is at an intermediate level of consumption. In this mode, the
//  chip is placed in halt mode waiting for instructions via SPI. This mode is
//  dedicated to chip configuration using high level commands such as
//  SetPacketType(...). By default, after battery insertion or reset operation
//  (pin NRESET goes low), the chip will enter in STDBY_RC mode running with a
//  13 MHz RC clock
//
//  Parameters
//  ----------
...
//  this command can be issued at any time and the device returns the status of
//  the device. The command GetStatus() is not strictly necessary since device
//  returns status information also on command bytes.
//
//  Parameters:
//  none
//
...
//----------------------------------------------------------------------------------------------------------------------------
//  The BUSY line is mandatory to ensure the host controller is ready to accept
//  SPI commands. When BUSY is high, the host controller must wait until it goes
//  down again before sending another command.
//
//  Parameters:
//  none
//
...
//
//----------------------------------------------------------------------------------------------------------------------------
void SX126x::SetBufferBaseAddress(uint8_t txBaseAddress,
...
//
//----------------------------------------------------------------------------------------------------------------------------
void SX126x::SetPaConfig(uint8_t paDutyCycle, uint8_t hpMax, uint8_t deviceSel,
                         uint8_t paLut) {
    uint8_t buf[4];
...
//----------------------------------------------------------------------------------------------------------------------------
//  The OCP is configurable by steps of 2.5 mA and the default value is
//  re-configured automatically each time the function SetPaConfig(...) is
//  called. If the user wants to adjust the OCP value, it is necessary to change
//  the register as a second step after calling the function SetPaConfig.
//
//  Parameters:
...
//
//----------------------------------------------------------------------------------------------------------------------------
void SX126x::SetDioIrqParams(uint16_t irqMask, uint16_t dio1Mask,
...
//----------------------------------------------------------------------------------------------------------------------------
//  In LoRa mode, when going into Rx, the modem will lock as soon as a LoRa®
//  symbol has been detected which may lead to false detection. This phenomena
//  is quite rare but nevertheless possible. To avoid this, the command
//  SetLoRaSymbNumTimeout can be used to define the number of symbols which will
//  be used to validate the correct reception of a packet.
//
//  Parameters:
//  0:      validate the reception as soon as a LoRa® Symbol has been detected
//  1..255: When SymbNum is different from 0, the modem will wait for a total of
//  SymbNum LoRa® symbol to validate, or not, the
//          correct detection of a LoRa packet. If the various states of the
//          demodulator are not locked at this moment, the radio will generate
//          the RxTimeout IRQ.
//
//
//  Return value:
...
//
//----------------------------------------------------------------------------------------------------------------------------
void SX126x::SetModulationParams(uint8_t spreadingFactor, uint8_t bandwidth,
                                 uint8_t codingRate,
...
//----------------------------------------------------------------------------------------------------------------------------
//  The command SetTx() sets the device in transmit mode. When the last bit of
//  the packet has been sent, an IRQ TX_DONE is generated. A TIMEOUT IRQ is
//  triggered if the TX_DONE IRQ is not generated within the given timeout
//  period. The chip goes back to STBY_RC mode after a TIMEOUT IRQ or a TX_DONE
//  IRQ. he timeout duration can be computed with the formula: Timeout duration
//  = Timeout * 15.625 μs
//
//  Parameters:
//  0: Timeout disable, Tx Single mode, the device will stay in TX Mode until
//  the packet is transmitted other: Timeout in milliseconds, timeout active,
//  the device remains in TX mode. The maximum timeout is then 262 s.
//
//
//
//  Return value:
...
uint8_t SX126x::ReadBuffer(uint8_t *rxData, uint8_t *rxDataLen,
    ...
    spi_write_blocking(SPI_Peripheral, &temp1, 1);
    spi_write_blocking(SPI_Peripheral, &offset, 1);
    spi_write_blocking(SPI_Peripheral, &temp1, 1);

    for (uint16_t i = 0; i < *rxDataLen; i++) {
        spi_write_read_blocking(SPI_Peripheral, &temp2, &rxData[i], 1);
    }
...
uint8_t SX126x::WriteBuffer(uint8_t *txData, uint8_t txDataLen) {
    ...
    spi_write_blocking(SPI_Peripheral, &temp1, 1);
    spi_write_blocking(SPI_Peripheral, 0, 1);
    // Serial.print(" 0 ");
    for (uint16_t i = 0; i < txDataLen; i++) {
        // Serial.print(txData[i]);
        // Serial.print(" ");
        spi_write_blocking(SPI_Peripheral, &txData[i], 2);
    }
    gpio_put(SX126x_SPI_SELECT, true);
    // Serial.println("");
    while (gpio_get(SX126x_BUSY))
...
void SX126x::SPItransfer(uint8_t cmd, bool write, uint8_t *dataOut,
    ...
    // send command byte
    spi_write_blocking(SPI_Peripheral, &cmd, 1);
    ...
    if (write) {
        ...
        for (uint8_t n = 0; n < numBytes; n++) {
            uint8_t in = 0;
            spi_write_read_blocking(SPI_Peripheral, &in, &dataOut[n], 1);
        ...
        uint8_t in = 0;
        spi_write_read_blocking(SPI_Peripheral, &in, SX126X_CMD_NOP, 1);
        ////Serial.println((SX126X_CMD_NOP, HEX));
        // Serial.print(" DataIn: ");

        for (uint8_t n = 0; n < numBytes; n++) {
            spi_write_read_blocking(SPI_Peripheral, &dataIn[n], SX126X_CMD_NOP,
                                    1);
            ////Serial.println((SX126X_CMD_NOP, HEX));
