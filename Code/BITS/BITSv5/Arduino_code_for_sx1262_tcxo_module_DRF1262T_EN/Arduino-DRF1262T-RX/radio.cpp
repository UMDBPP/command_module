 ...
 */
void RadioSetModem( uint8_t modem );
...
void RadioSetMaxPayloadLength( uint8_t modem, uint8_t max )
{
    if( modem == MODEM_LORA )
    {
...
void RadioOnTxTimeoutIrq( void )
{
    if( ( RadioEvents_Ptr != NULL ) && ( RadioEvents_Ptr->TxTimeout != NULL ) )
    {
        RadioEvents_Ptr->TxTimeout( );
    }
}

void RadioOnRxTimeoutIrq( void )
{
    if( ( RadioEvents_Ptr != NULL ) && ( RadioEvents_Ptr->RxTimeout != NULL ) )
    {
        RadioEvents_Ptr->RxTimeout( );
    }
}
...
void RadioIrqProcess( void )
{
        ...
        {
            if( ( RadioEvents_Ptr != NULL ) && ( RadioEvents_Ptr->TxDone != NULL ) )
            {
                RadioEvents_Ptr->TxDone( );
            }
        }
        ...
        if( ( irqRegs & IRQ_RX_DONE ) == IRQ_RX_DONE )
        {
            ...
            if( ( RadioEvents_Ptr != NULL ) && ( RadioEvents_Ptr->RxDone != NULL ) )
            {
                RadioEvents_Ptr->RxDone( RadioRxPayload, size, RadioPktStatus.Params.LoRa.RssiPkt, RadioPktStatus.Params.LoRa.SnrPkt );
            }
        }

        if( ( irqRegs & IRQ_CRC_ERROR ) == IRQ_CRC_ERROR )
        {
            if( ( RadioEvents_Ptr != NULL ) && ( RadioEvents_Ptr->RxError ) )
            {
                RadioEvents_Ptr->RxError( );
            }
        }

        if( ( irqRegs & IRQ_CAD_DONE ) == IRQ_CAD_DONE )
        {
            if( ( RadioEvents_Ptr != NULL ) && ( RadioEvents_Ptr->CadDone != NULL ) )
            {
                RadioEvents_Ptr->CadDone( ( ( irqRegs & IRQ_CAD_ACTIVITY_DETECTED ) == IRQ_CAD_ACTIVITY_DETECTED ) );
            }
        }
