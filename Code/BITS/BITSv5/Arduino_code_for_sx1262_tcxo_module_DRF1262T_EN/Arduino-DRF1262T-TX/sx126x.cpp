...
void SX126xInit( void )
{
    ...
    SX126xCalibrate( calibParam.Value );
#endif
...
uint8_t SX126xGetOperatingMode( void )
{
...
uint32_t SX126xGetRandom( void )
{
    ...
    SX126xSetStandby( STDBY_RC );
    ret_val |= buf[0];
    ret_val = (ret_val<<8)+buf[1] ;
    ret_val = (ret_val<<8)+buf[2] ;
    ret_val = (ret_val<<8)+buf[3] ;
   
    return ret_val;
}

void SX126xSetSleep( uint8_t sleepConfig )
{

    SX126xWriteCommand( RADIO_SET_SLEEP, &sleepConfig, 1 );
    OperatingMode = MODE_SLEEP;
}

void SX126xSetStandby( uint8_t standbyConfig )
{
...
void SX126xSetDio3AsTcxoCtrl( uint8_t tcxoVoltage, uint32_t timeout )
{
    uint8_t buf[4];
...
void SX126xSetTxParams( int8_t power, uint8_t rampTime )
{
    uint8_t buf[2];
...
void SX126xSetModulationParams( void *input_ptr )
{
    uint8_t n;
    uint32_t tempVal = 0;
    uint8_t buf[8] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    ModulationParams_t* modulationParams = (ModulationParams_t*)input_ptr;
...
void SX126xSetPacketParams( void* input_ptr )
{
    uint8_t n;
    uint8_t crcVal = 0;
    uint8_t buf[9] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    PacketParams_t* packetParams = (PacketParams_t*)input_ptr;
    // Check if required configuration corresponds to the stored packet type
    // If not, silently update radio packet type
...
void SX126xSetCadParams( uint8_t cadSymbolNum, uint8_t cadDetPeak, uint8_t cadDetMin, uint8_t cadExitMode, uint32_t cadTimeout )
{
    uint8_t buf[7];
...
void SX126xGetPacketStatus( void* input_ptr )
{
    uint8_t status[3];
     PacketStatus_t *pktStatus = (PacketStatus_t *)input_ptr;
...
uint16_t SX126xGetDeviceErrors( void )
{
    uint16_t error;
