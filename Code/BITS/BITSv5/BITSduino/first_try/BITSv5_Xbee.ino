...
bool xbeeSend(uint32_t TargetSL, uint8_t *payload) {
  ...
  ZBTxRequest zbTx =
      ZBTxRequest(TargetAddress, payload, xbeeSendBufSize); // Assembles Packet
...
void xbeeRead() {
  ...
  if (xbee.getResponse().isAvailable()) { // got something
    if (xbee.getResponse().getApiId() ==
        ZB_RX_RESPONSE) { // got a TxRequestPacket
      xbee.getResponse().getZBRxResponse(rx);
      uint32_t incominglsb =
          rx.getRemoteAddress64().getLsb(); // ID the message sender
      logprint_str("IncPckFrom ");
      logprintln_uint32(incominglsb);
      if (rx.getPacketLength() >= xbeeRecBufSize) {
        logprint_str("Oversized Message: ");
        logprintln_int(rx.getPacketLength());
      }
      memset(xbeeRecBuf, 0, xbeeRecBufSize); // Clears old buffer
      memcpy(xbeeRecBuf, rx.getData(),
             rx.getPacketLength()); // Safely fills buffer

      // This separates out the actual methods used to interpret XBee data,
      // preventing payloads from commanding themselves As a sidenote, a payload
      // commanding itself would invlove some fun AI, and probably also disaster
...
// Each XBee has its own function set, which allows more control over what
// payloads are allowed to do.
//  ex. Commands are locked to specific senders, so while command might be able
//  to send a trigger, a secondary could not
//
// Without a proper naming scheme, the current XBee's are named Bits, Ground,
// Blue and Wire
...
void processGroundMessage() {
  ...
  // Ping Pong Test
  if (strstr((char *)xbeeRecBuf, "ping")) { // Received "ping", attempt to
                                            // "pong"
    OutputSerial.println("groundPingPong");
    logprintln_str("groundPingPong");
  ...
  if (strstr((char *)xbeeRecBuf, "TG")) { // If contains TG tag, commence
    ...
    logprintln_str("TGgnd");
    String("TGgndReq").getBytes(xbeeSendBuf, xbeeSendBufSize);
    xbeeSend(GroundSL, xbeeSendBuf); // Let Ground XBee know request to TX via
                                     // Iridium has been filed
    downlinkData = true;
    // strcat(downlinkMessage2,(char*)xbeeRecBuf); //NEVER EVER USE THIS

    // Append the message to Iridium's packet in a safe manner. Excess bits will
    // be deleted
    strncat(downlinkMessage2, (char *)xbeeRecBuf,
            downlinkMessageSize - strlen(downlinkMessage2) - 1);
    logprint_str("GNDDownAppend");
...
void processWireMessage() {
  ...
  logprintln_str("RecWire");

  // To Ground Relay
  if (strstr((char *)xbeeRecBuf, "TG")) {
    OutputSerial.println("TG_Wire");
    logprintln_str("TG_Wire");
    downlinkData = true;
    // strcat(downlinkMessage2,(char*)xbeeRecBuf);//DELETE on TEST
    strncat(downlinkMessage2, (char *)xbeeRecBuf,
            downlinkMessageSize - strlen(downlinkMessage2) - 1);
    logprint_str("WireDownAppend");
    logprintln_str(downlinkMessage2);
  }
...
void processGHOULMessage() {
  ...
  logprintln_str("RecGHOUL");

  if (strstr((char *)xbeeRecBuf, "Ack")) {
    OutputSerial.println("Ack_GHOUL");
    logprintln_str("Ack_GHOUL");
    downlinkData = true;
    // strcat(downlinkMessage2,(char*)xbeeRecBuf);//DELETE on TEST
    strncat(downlinkMessage2, (char *)xbeeRecBuf,
            downlinkMessageSize - strlen(downlinkMessage2) - 1);
    logprint_str("GHOULDownAppend");
    logprintln_str(downlinkMessage2);
  }
}
