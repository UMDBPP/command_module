...
void xbeeRead(){
      ...
      if (xbee.getResponse().isAvailable()) { //got something
        if (xbee.getResponse().getApiId() == ZB_RX_RESPONSE) { //got a TxRequestPacket
            ...
            // Clear old buffer, and then write data to the buffer
            memset(xbeeRecBuf, 0, xbeeRecBufSize);
...
//  ex. Commands are locked to specific senders, so while BITS might be able to send a trigger command to a
//      drop payload, a secondary payload couldn't do that.
// 
...
void processGroundMessage(){
    ...
    OutputSerial.println("RecFromGND");
...
void processBlueMessage(){
    ...
    OutputSerial.println("RecFromBlue");
    logprintln("RecFromBlue");
